...
D3DXVECTOR3 CInput::VectorMoveJoyStick(bool bleftandright, int nNum)
{
	...
	if (VectorMove.x >= 0)
	{
		if (VectorMove.x <= 250.0f)
		{
			VectorMove = D3DXVECTOR3(0.0f, VectorMove.y, VectorMove.z);
		}
	}
	else
	{
	...
	if (VectorMove.y >= 0)
	{
		if (VectorMove.y <= 250.0f)
		{
			VectorMove = D3DXVECTOR3(VectorMove.x, 0.0f, VectorMove.z);
		}
	}
	else
	{
